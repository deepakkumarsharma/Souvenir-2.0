        ...
        <div class="s_data shrink">
            <div class="content id_first-slide-text">
            	<form class="register" method="post" accept-charset="utf-8" action="<?php echo base_url();?>Souvenir-2.0/index.php/registration">
					...
					<input type="password" class="pass" name="Password" onchange="form.Passconf.pattern = this.value;" placeholder="Password"  pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Minimum 8 letters or numbers." required /><br>
					<input type="password" class="cPass" name="Passconf" placeholder="Confirm Password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Re-type Password" required /><br><br>  
